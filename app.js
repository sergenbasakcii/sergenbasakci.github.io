...
if(window.pdfjsLib&&window.pdfjsLib.GlobalWorkerOptions){window.pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.8.69/pdf.worker.min.js'}
...
const show=m=>{toast.textContent=m;toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1400)}
...
(function init(){const qs=new URLSearchParams(location.search);if(qs.get('hard')==='1'&&'caches'in window){caches.keys().then(keys=>keys.forEach(k=>caches.delete(k)))}screen.textContent='Hazır';wpmVal.textContent=wpm.value})()
})();