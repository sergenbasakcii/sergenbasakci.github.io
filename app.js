...
const screen=$('screen'),wpm=$('wpm'),wpmVal=$('wpmVal'),chunkSel=$('chunk'),orp=$('orp'),breath=$('breath'),prog=$('progress'),bar=$('bar'),input=$('input'),pdfFile=$('pdfFile'),drop=$('drop'),imgFile=$('imgFile')
const startBtn=$('start'),pauseBtn=$('pause'),back10=$('back10'),fwd10=$('fwd10'),fsize=$('fsize'),color=$('color'),sound=$('sound'),themeBtn=$('theme'),installBtn=$('install')
const tabs=[...document.querySelectorAll('.tab')],panels={paste:$('panel-paste'),pdf:$('panel-pdf'),ocr:$('panel-ocr')},toast=$('toast')
...
async function readPdf(f){input.value='PDF okunuyor...';try{const buf=await f.arrayBuffer();const pdf=await pdfjsLib.getDocument({data:buf}).promise;let txt='';for(let p=1;p<=pdf.numPages;p++){const page=await pdf.getPage(p);const c=await page.getTextContent();const s=c.items.map(i=>('str'in i)?i.str:(i?.text||'')).join(' ');txt+='\\n'+s}input.value=txt||'(Metin bulunamadı. Görüntü tabanlı olabilir; OCR sekmesini deneyin.)';tabTo('paste');rebuild();render(S.chunks[0]||['Hazır']);show('PDF yüklendi')}catch(e){input.value='PDF okunamadı.';rebuild();show('PDF okunamadı')}}
let ocrReady=false
$('enableOcr').addEventListener('click',()=>{if(ocrReady)return;const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js';s.onload=()=>{ocrReady=true;show('OCR etkin');};document.body.appendChild(s)})
imgFile.addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f){show('Dosya seçin');return}if(!ocrReady){show('Önce OCR\'yi etkinleştirin');return}input.value='OCR çalışıyor...';try{if(f.type==='application/pdf'){await readPdf(f)}else{const b=await f.arrayBuffer();const {createWorker}=Tesseract;const worker=await createWorker('tur');const {data:{text}}=await worker.recognize(new Blob([b]));await worker.terminate();input.value=text||'(Metin çıkmadı)';tabTo('paste');rebuild();render(S.chunks[0]||['Hazır']);show('OCR tamam')}}catch(err){input.value='OCR başarısız';rebuild();show('OCR başarısız')}})
...
const setSize=()=>{screen.style.fontSize=fsize.value+'px'}
const setColor=()=>{S.accent=color.value;document.documentElement.style.setProperty('--accent',S.accent)}
setSize();setColor();wpmVal.textContent=wpm.value;render(['Hazır']);updateUI()
let theme='dark';themeBtn.addEventListener('click',()=>{theme=theme==='dark'?'light':'dark';document.documentElement.setAttribute('data-theme',theme)})
let deferred=null;window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferred=e;installBtn.style.display='inline-block'});installBtn.addEventListener('click',async()=>{if(!deferred)return;deferred.prompt();await deferred.userChoice;deferred=null;installBtn.style.display='none'})
